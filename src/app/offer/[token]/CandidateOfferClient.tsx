...
import { Check, X, FileText, Calendar, DollarSign, MessageCircle, ArrowRight, Loader2 } from "lucide-react";
...
export function CandidateOfferClient({ offer, token }: CandidateOfferClientProps) {
  ...
  const [view, setView] = useState<"MAIN" | "DECLINE" | "QUESTION">("MAIN");
    ...
    try {
        if (action === "ACCEPT") {
            ...
            if (data.signatureUrl) {
                // Redirect candidate directly into the DocuSign embedded Signing experience
                window.location.href = data.signatureUrl;
            } else {
                alert(data.error || "Failed to initiate signature session.");
                setIsSubmitting(false);
            }
        } else if (action === "DECLINE") {
            alert("Offer Declined. Notifying recruiter.");
            setIsSubmitting(false);
            router.refresh();
        } else {
            alert("Question submitted to Recruiter!");
            setView("MAIN");
            setNotes("");
            setIsSubmitting(false);
        }
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="bg-[#111111] w-full pt-16 pb-32 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 text-center">
          <p className="text-indigo-400 font-bold tracking-widest uppercase text-sm mb-4 animate-in fade-in duration-700">Official Offer</p>
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-16 relative z-20">
                    ...
                    <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div className="bg-white rounded-3xl p-6 shadow-lg shadow-gray-100 border border-gray-100 flex items-center gap-4">
                             ...
                             <div>
                                 ...
                                 <p className="text-lg font-bold text-gray-900">{new Date(offer.startDate).toLocaleDateString(undefined, { weekday: 'short', month: 'long', day: 'numeric'})}</p>
                             </div>
                         </div>
                         <div className="bg-white rounded-3xl p-6 shadow-lg shadow-gray-100 border border-gray-100 flex items-center gap-4">
                             ...
                             <div>
                                 ...
                                 <p className="text-lg font-bold text-gray-900">{new Date(offer.expirationDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric'})}</p>
                             </div>
                         </div>
                    </div>
        ...
        {view === "MAIN" && activeTab === "DOCUMENT" && (
            <div className="bg-white rounded-3xl p-8 shadow-xl shadow-gray-200/50 border border-gray-100 min-h-[600px] animate-in fade-in zoom-in-95 duration-300">
                 <div className="prose prose-gray max-w-none" dangerouslySetInnerHTML={{ __html: offer.template?.contentHtml || "<p>Rendered document will appear here.</p>" }} />
            </div>
        )}
        ...
        {view === "QUESTION" && (
            <div className="bg-white rounded-3xl p-10 shadow-xl shadow-gray-200/50 border border-gray-100 max-w-2xl mx-auto animate-in fade-in zoom-in-95 duration-300">
                ...
                <textarea
                    ...
                    className="w-full h-32 p-4 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none mb-6"
                />
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleAction("QUESTION")} disabled={isSubmitting || !notes.trim()} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-bold shadow-md shadow-indigo-500/20 transition-all flex items-center justify-center gap-2">
