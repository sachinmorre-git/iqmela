  ...
  return createPortal(
    <>
      ...
      <div className="fixed top-0 right-0 z-50 h-full w-full max-w-5xl bg-white dark:bg-zinc-900 border-l border-gray-200 dark:border-zinc-800 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 sticky top-0 z-20">
          <div className="flex items-center gap-6 w-full mr-6">
            
            {/* Left: Schedule Info */}
            <div className="shrink-0 min-w-[200px] border-r border-gray-200 dark:border-zinc-800 pr-6">
              ...
              <p className="text-xs text-gray-500 dark:text-zinc-400 mt-1 uppercase tracking-wide font-semibold">
                Round {stage.stage.stageIndex + 1} • {stage.stage.roundType.replace(/_/g, " ")}
              </p>
            </div>

            {/* Right: Candidate Info (Merged from body) */}
            <div className="flex-1 flex items-center justify-between gap-4 bg-gray-50/50 dark:bg-zinc-800/30 rounded-xl px-4 py-2">
              <div className="flex items-center gap-6">
                <div>
                  <p className="font-bold text-base text-gray-900 dark:text-white">{candidateName}</p>
                  {matchScore != null && (
          ...
          <button type="button" onClick={onClose} className="shrink-0 w-8 h-8 rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-800 flex items-center justify-center text-gray-400 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {!success && !(isCompleted && !isAI && stage?.interviewId) && !isBGV && (
          <div className="px-6 py-4 border-t border-gray-100 dark:border-zinc-800 flex items-center justify-center gap-3">
            <button
              ...
              disabled={isPending}
              className="px-6 py-3 rounded-xl text-lg font-bold text-gray-500 hover:text-gray-700 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors"
            >
            ...
            {isAI && isAvailable && (
              <button type="button" onClick={handleSendAiInvite} disabled={isPending}
                className="px-6 py-3 rounded-xl text-lg font-bold bg-pink-600 text-white hover:bg-pink-700 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2">
            ...
            {!isAI && isScheduled && !isEditing && (
              <>
                <button type="button" onClick={() => setIsEditing(true)} disabled={isPending}
                  className="px-6 py-3 rounded-xl text-lg font-bold border border-gray-200 dark:border-zinc-700 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-all inline-flex items-center gap-2">
                  <Edit3 className="w-5 h-5" /> Reschedule
                </button>
                <button type="button" onClick={handleMarkComplete} disabled={isPending}
                  className="px-6 py-3 rounded-xl text-lg font-bold bg-emerald-600 text-white hover:bg-emerald-700 transition-all shadow-sm disabled:opacity-50 inline-flex items-center gap-2">
            ...
            {!isAI && (isAvailable || isEditing) && mode === "quick" && (
              <button type="button" onClick={handleQuickSchedule}
                ...
                className="px-6 py-3 rounded-xl text-lg font-bold bg-rose-600 text-white hover:bg-rose-700 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2">
                {isPending ? <><Loader2 className="w-5 h-5 animate-spin" /> Scheduling…</> : <>🚀 {isEditing ? "Reschedule" : "Schedule & Notify"}</>}
              </button>
            )}
            ...
            {!isAI && (isAvailable || isEditing) && mode === "poll" && (
              <div className="relative">
                <button type="button" onClick={handleSmartPoll}
                  ...
                  className="px-6 py-3 rounded-xl text-lg font-bold bg-pink-600 text-white hover:bg-pink-700 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2">
                  {isPending ? <><Loader2 className="w-5 h-5 animate-spin" /> Sending…</> : <>📨 Request Availability</>}
                </button>
              </div>
            )}
          </div>
