...
export default async function OrgAdminLayout({
  ...
  // We rely on Clerk Organizations for B2B RBAC.
  const isB2BUser = orgRole?.startsWith('org:')
  if (!isB2BUser) {
    redirect('/select-role')
  }

  // Define access based on the "Tesla-like" RBAC matrix
  const isOrgAdmin = orgRole === 'org:admin'
  const isBillingAdmin = orgRole === 'org:billing_admin'
  const isRecruiter = orgRole === 'org:recruiter'
  const isHiringManager = orgRole === 'org:hiring_manager'

  // Visibility logic
  const canSeePositions = isOrgAdmin || isRecruiter || isHiringManager
  const canSeeCandidates = isOrgAdmin || isRecruiter || isHiringManager
  const canSeeInterviews = true // general dashboard view
  const canSeeInvites = isOrgAdmin || isRecruiter
  const canSeeActivity = isOrgAdmin
  const canSeeTeam = isOrgAdmin
  const canSeeSettings = isOrgAdmin || isBillingAdmin
  // ── End Role Guard ──────────────────────────────────────────────────────
      ...
      <aside className="w-full md:w-64 lg:w-72 border-b md:border-b-0 md:border-r border-gray-200 dark:border-zinc-800 bg-gray-50/50 dark:bg-zinc-900/40 py-6 flex flex-col gap-6">
        ...
        <div className="px-6 mb-2">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-lg font-bold text-gray-900 dark:text-white tracking-tight">IQMela <span className="font-light text-zinc-400 text-sm">B2B</span></h2>
          </div>
          ...
          <OrganizationSwitcher 
            ...
            appearance={{
              elements: {
                rootBox: "w-full",
                organizationSwitcherTrigger: "w-full flex justify-between bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 text-gray-900 dark:text-zinc-100 rounded-xl p-2.5 hover:bg-gray-50 dark:hover:bg-zinc-800/80 transition-colors shadow-sm",
                organizationSwitcherTriggerIcon: "text-zinc-500",
                organizationPreviewTextContainer: "dark:text-zinc-200",
                userPreviewTextContainer: "dark:text-zinc-200",
                popoverContent: "dark:bg-zinc-900 dark:border-zinc-800 rounded-xl shadow-2xl",
                organizationPreviewMainIdentifier: "dark:text-white font-medium",
