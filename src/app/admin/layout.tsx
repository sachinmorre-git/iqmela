...
export default async function GlobalAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <>
      ...
      <div className="mb-2 px-4 group-[.collapsed]:hidden">
        <div className="flex items-center gap-3 mb-1">
          ...
          />
          <h2 className="text-xl font-black tracking-tighter text-white truncate">IQMela <span className="font-light text-rose-400 text-sm tracking-wide ml-1 uppercase">Platform</span></h2>
        </div>
        <p className="text-[10px] uppercase font-bold tracking-widest text-zinc-500 ml-12">System Control</p>
      </div>
      
      {/* Collapsed view icon */}
      <div className="hidden group-[.collapsed]:flex mb-2 px-4 justify-center">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/brand/icon/iq-icon.svg"
          alt="IQMela"
      ...
      <nav className="flex flex-col gap-1.5 h-full px-2">
        {canSeeDashboard && (
        ...
        <div className="mt-auto pt-6 border-t border-zinc-900 group-[.collapsed]:px-0 px-3">
           <div className="flex items-center gap-3 justify-center group-[]:justify-start">
             <div className="w-8 h-8 bg-zinc-800 rounded-full flex items-center justify-center font-bold text-xs uppercase text-zinc-300 shrink-0">
                {sysRole.replace('sys:', '').slice(0, 2)}
             </div>
             <div className="group-[.collapsed]:hidden">
                <p className="text-xs font-bold text-white uppercase tracking-wider truncate">{sysRole.replace('sys:', '')}</p>
                <p className="text-[10px] text-zinc-500 truncate">Authenticated Proxy</p>
             </div>
           </div>
        </div>
      </nav>
  ...
  return (
    <ResizableSidebarLayout 
      ...
      className="flex flex-col md:flex-row min-h-screen w-full bg-black text-white selection:bg-rose-500/30 overflow-hidden relative"
      sidebarClassName="border-b md:border-b-0 md:border-r border-rose-900/30 bg-zinc-950 shadow-2xl z-10"
      mainClassName="flex-1 flex flex-col bg-[#050505] overflow-y-auto relative z-0 min-w-0"
    >
