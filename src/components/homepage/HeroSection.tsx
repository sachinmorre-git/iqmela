...
function HudMockup() {
  return (
    <div className="relative w-full max-w-lg mx-auto select-none pointer-events-none">
      {/* Outer frame */}
      <div className="rounded-2xl border border-zinc-700/50 bg-gradient-to-b from-zinc-900 to-zinc-950 p-4 shadow-2xl shadow-indigo-900/20">
        {/* Top bar */}
        ...
        <div className="bg-zinc-800/60 rounded-xl h-32 flex items-center justify-center mb-3 relative overflow-hidden">
          ...
          <div className="relative flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-indigo-600 flex items-center justify-center text-sm font-black">AC</div>
            <div>
          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-end gap-0.5">
            {[4,7,5,9,6,8,3,7,5,8,4,6].map((h, i) => (
              <div
                key={i}
                className="w-1 bg-indigo-400/70 rounded-full"
                style={{
                  height: `${h * 2}px`,
        ...
        {/* Signal rows */}
        {[
          { label: "Communication",   score: 82, color: "bg-indigo-500" },
          { label: "Confidence",      score: 71, color: "bg-violet-500" },
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-4 py-24 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-indigo-700/20 blur-3xl rounded-full" />
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-violet-700/15 blur-3xl rounded-full" />
      </div>

      <div className="relative z-10 max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        {/* Copy */}
        <div className="text-center lg:text-left space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-indigo-500/30 bg-indigo-500/10 text-indigo-400 text-xs font-bold mb-2">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
            <Link href="/sign-up">
              <button className="inline-flex items-center gap-2 px-7 py-3.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-black text-sm transition-all shadow-lg shadow-indigo-600/30 hover:shadow-indigo-600/50 hover:-translate-y-0.5">
                Get Started Free <ArrowRight className="w-4 h-4" />
              </button>
            </Link>
            <a href="mailto:demo@iqmela.com">
              <button className="inline-flex items-center gap-2 px-7 py-3.5 rounded-2xl bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-bold text-sm border border-zinc-700 transition-all hover:-translate-y-0.5">
                Book a Demo
              </button>
            </a>
          </div>
