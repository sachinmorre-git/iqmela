...
export function OrbDisplay({ phase }: { phase: VisualPhase }) {
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-br from-zinc-900 via-zinc-900 to-indigo-950/30" />
        <div className="relative z-10 flex flex-col items-center gap-2">
          <div
            className={`relative w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden border-4 transition-all duration-700 ${
              phase === "asking"
                ? "border-indigo-500 scale-105 shadow-[0_0_40px_rgba(99,102,241,0.5)]"
                : phase === "listening"
          ...
          >
            ...
            >
              <div className="w-12 h-12 md:w-16 md:h-16 bg-white/10 rounded-full flex items-center justify-center backdrop-blur-md">
                <div
                  className={`w-6 h-6 md:w-8 md:h-8 rounded-full bg-indigo-100 transition-all duration-[400ms] ${
                    phase === "asking"
        ...
        {/* Asking glow sweep */}
        {phase === "asking" && (
          <div className="absolute inset-0 bg-gradient-to-t from-indigo-600/10 to-transparent pointer-events-none animate-pulse" />
        )}
      </div>
      {/* Provider label */}
